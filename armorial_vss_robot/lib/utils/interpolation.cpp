...
double Interpolation::Step(double xValues[], double yValues[], int numValues,
                           double pointX, double threshold) {
  // extremos
  if (pointX <= xValues[0])
    return yValues[0];
  ...
  auto t = (pointX - xValues[i]) /
           (xValues[i + 1] - xValues[i]); // punto relativo en el intervalo
  return t < threshold ? yValues[i] : yValues[i + 1];
}

double Interpolation::Linear(double xValues[], double yValues[], int numValues,
  ...
  if (pointX <= xValues[0]) {
    ...
    auto t = (pointX - xValues[numValues - 2]) /
             (xValues[numValues - 1] - xValues[numValues - 2]);
...
double Interpolation::SmoothStep(double xValues[], double yValues[],
                                 int numValues, double pointX, bool trim) {
  if (trim) {
    if (pointX <= xValues[0])
      return yValues[0];
    if (pointX >= xValues[numValues - 1])
...
double Interpolation::CatmullSpline(double xValues[], double yValues[],
                                    int numValues, double pointX, bool trim) {
  if (trim) {
    if (pointX <= xValues[0])
      return yValues[0];
  ...
  if (i == 0) {
    ...
    m0 = (yValues[numValues - 1] - yValues[numValues - 3]) /
         (xValues[numValues - 1] - xValues[numValues - 3]);
    m1 = (yValues[numValues - 1] - yValues[numValues - 2]) /
         (xValues[numValues - 1] - xValues[numValues - 2]);
  } else {
...
double Interpolation::ConstrainedSpline(double xValues[], double yValues[],
  ...
  auto b =
      (y0 - y1 - c * (x0 * x0 - x1 * x1) - d * (x0 * x0 * x0 - x1 * x1 * x1)) /
      (x0 - x1);
...
double Interpolation::getFirstDerivate(double x[], double y[], int n, int i) {
  ...
  if (i == 0) {
    ...
    if ((x[i + 1] - x[i]) / (y[i + 1] - y[i]) * (x[i] - x[i - 1]) /
      ...
      fd1_x = 2.0f / ((x[i + 1] - x[i]) / (y[i + 1] - y[i]) +
                      (x[i] - x[i - 1]) / (y[i] - y[i - 1]));
    }
  }
  return fd1_x;
...
double Interpolation::getLeftSecondDerivate(double x[], double y[], int n,
                                            int i) {
...
double Interpolation::getRightSecondDerivate(double x[], double y[],
                                             int numValues, int i) {
