#ifndef ARMORIAL_SUASSUNA_CRC_H
#define ARMORIAL_SUASSUNA_CRC_H
...
static uint16_t compute_crc16cdma2000_byte(const uint16_t &initialCRCValue,
                                           const char *data,
                                           const size_t &len) {
  if (data == NULL)
    return initialCRCValue;
...
static bool validate_feedbackpacket_crc(FeedbackPacket packet) {
  uint16_t packetCRC = packet.crc;
  packet.crc = 0;
  return (packetCRC == compute_crc16cdma2000_byte(CRC_INITIAL_VALUE,
                                                  (char *)&packet,
